...
import { useGetListingDraftEvidenceQuery } from '@listforge/api-rtk';
...
import type { ListingDraftDto } from '@listforge/api-types';
import type {
...
interface EvidencePanelProps {
  draftId: string | null;
  draft: ListingDraftDto | null;
}

export function EvidencePanel({ draftId, draft }: EvidencePanelProps) {
  const { data, isLoading, error } = useGetListingDraftEvidenceQuery(draftId!, {
    skip: !draftId,
  });
        ...
        <div className="flex-1 overflow-y-auto">
          <TabsContent value="pricing" className="m-0 p-4">
            <PricingCompsTab
              ...
              researchSnapshot={draft?.researchSnapshot ?? null}
            />
          </TabsContent>

          <TabsContent value="attributes" className="m-0 p-4">
            <AttributesTab attributes={draft?.attributes ?? []} />
          </TabsContent>
