...
function MarketplacesPage() {
	...
	const [exchangeEbayCode, { isLoading: isExchangingEbay }] =
		useExchangeEbayCodeMutation()
	const [exchangeAmazonCode, { isLoading: isExchangingAmazon }] =
		useExchangeAmazonCodeMutation()
	const [exchangeFacebookCode, { isLoading: isExchangingFacebook }] =
		useExchangeFacebookCodeMutation()
	...
	const [isGettingFacebookUrl, setIsGettingFacebookUrl] = useState(false)
	...
	const isExchanging = isExchangingEbay || isExchangingAmazon || isExchangingFacebook
	...
	// Handle eBay/Facebook OAuth callback
	// Both platforms use code+state params. We try eBay first, then Facebook on state mismatch.
	useEffect(() => {
		if (code && state && !spapi_oauth_code) {
			...
			exchangeEbayCode({ code, state })
				...
				.catch((ebayErr) => {
					// Check if this is a state mismatch error (wrong marketplace)
					const errorMessage = ebayErr?.data?.message || ebayErr?.message || ''
					const isStateMismatch = errorMessage.toLowerCase().includes('state') ||
						errorMessage.toLowerCase().includes('invalid') ||
						errorMessage.toLowerCase().includes('mismatch')

					if (isStateMismatch) {
						// Try Facebook exchange
						exchangeFacebookCode({ code, state })
							...
							.catch((fbErr) => {
								console.error('Failed to exchange code for any marketplace:', { ebayErr, fbErr })
								showError('Failed to connect marketplace account. Please try again.')
								clearSearchParams()
							})
					} else {
						console.error('Failed to exchange eBay code:', ebayErr)
						showError('Failed to connect eBay account. Please try again.')
						clearSearchParams()
					}
				})
		}
	}, [code, state, spapi_oauth_code, exchangeEbayCode, exchangeFacebookCode, navigate, refetch])
	...
	useEffect(() => {
		...
		if (spapi_oauth_code && state) {
			...
			if (!selling_partner_id) {
				...
				console.error(
					'Amazon OAuth callback missing selling_partner_id parameter'
				)
				clearSearchParams()
				return
			}
	...
	// Note: Facebook OAuth callback is handled by the eBay handler above
	// Both use code+state params. The backend validates the state signature
	// and extracts the marketplace type from it. If state was created for eBay,
	// the Facebook exchange will fail (and vice versa). This is intentional -
	// only the correct marketplace handler will succeed.
	//
	// We use a single handler for code+state (eBay handler) which tries eBay first.
	// If that fails due to state mismatch, we then try Facebook.
	// This is handled by wrapping both in the same useEffect with fallback logic.
	...
	const handleConnectFacebook = async () => {
		try {
			setIsGettingFacebookUrl(true)
			const result = await dispatch(
				api.endpoints.getFacebookAuthUrl.initiate(undefined)
			).unwrap()
			if (result?.authUrl) {
				window.location.href = result.authUrl
			}
			...
			<div className="flex items-center justify-center py-12">
				<div className="text-center">
					...
					<p className="text-muted-foreground">
						...
						{isExchangingFacebook && 'Connecting your Facebook account...'}
					</p>
				</div>
			</div>
		)
	...
	return (
		<AppContent
			...
			description="Connect your marketplace accounts to start publishing listings"
			breadcrumbs={[
				{
					label: 'Settings',
					href: '/settings',
					...
					<div className="space-y-4">
						...
						<div className="flex items-center justify-between p-4 border rounded-lg">
							<div className="flex items-center gap-4">
								...
								<div>
									<h3 className="font-medium">Facebook Marketplace</h3>
									<p className="text-sm text-muted-foreground">
							...
							<Button
								...
								disabled={isGettingFacebookUrl}
							>
								{isGettingFacebookUrl ? (
									<>
										<Loader2 className="mr-2 h-4 w-4 animate-spin" />
										Connecting...
									</>
											...
											<AlertDialogContent>
												<AlertDialogHeader>
													...
													<AlertDialogDescription>
														...
														{getMarketplaceName(account.marketplace)} account?
